...
async function startMic(){
    initAudio();
    try{
        // Only ask for permission once per session
...
function stopMic(){
    ...
    // Disconnect analyser but keep stream alive so we don't re-prompt
    if(micSource && analyser){
        try{ micSource.disconnect(analyser); }catch(e){}
    }
    analyser=null;
}
function destroyMic(){
...
function switchView(viewId){
    // Clean up mic when leaving tone practice so the indicator turns off
    if(document.querySelector('.view.active')?.id === 'tonePracticeView'){
        destroyMic();
    }
